    ...
    <header className="sticky top-0 z-50 w-full bg-white border-b border-gray-200 shadow-sm">
      <div className="container mx-auto px-4 py-3 flex flex-wrap items-center justify-between gap-y-3">
        ...
        <div className="flex items-center gap-2 sm:gap-6 order-2 lg:order-3">
          <Link to="/account" className="flex flex-col items-center text-gray-600 hover:text-blue-600 transition-colors">
            <User className="w-6 h-6" />
            <span className="text-xs hidden sm:block mt-1 font-medium">Account</span>
          </Link>
          
          <Link to="/wishlist" className="flex flex-col items-center text-gray-600 hover:text-blue-600 transition-colors relative">
          ...
          <Link to="/cart" className="flex items-center gap-2 text-gray-600 hover:text-blue-600 transition-colors bg-gray-50 hover:bg-blue-50 px-3 py-2 rounded-lg border border-gray-100">
            <div className="relative">
              <ShoppingCart className="w-6 h-6" />
              {cartQuantity > 0 && (
                <span className="absolute -top-2 -right-2 bg-blue-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow-sm">
                  {cartQuantity}
                </span>
              )}
            </div>
          ...
          <div className="fixed inset-y-0 left-0 w-64 bg-white shadow-xl flex flex-col animate-in slide-in-from-left-full duration-300">
            ...
            <div className="flex-1 overflow-y-auto py-4">
              <nav className="flex flex-col gap-2 px-4">
                <Link to="/track" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 py-3 text-gray-700 hover:text-blue-600 border-b border-gray-50">
                  <Truck className="w-5 h-5 text-gray-400" /> Track Order
                </Link>
                <Link to="/help" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 py-3 text-gray-700 hover:text-blue-600 border-b border-gray-50">
                  <HelpCircle className="w-5 h-5 text-gray-400" /> Help Center
                </Link>
                <Link to="/buyer-protection" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 py-3 text-gray-700 hover:text-blue-600 border-b border-gray-50">
