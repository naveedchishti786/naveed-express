        ...
        <div className="bg-primary-50 border border-primary-100 rounded-xl p-4 mb-6 flex items-center gap-4">
          ...
          <div className="flex-1">
            {totalAmount >= FREE_SHIPPING_THRESHOLD ? (
              <p className="text-primary-800 font-medium">Congratulations! You qualify for <span className="font-bold">Free Shipping</span>.</p>
            ) : (
              <div>
                <p className="text-primary-800 font-medium mb-1">Add <span className="font-bold">${(FREE_SHIPPING_THRESHOLD - totalAmount).toFixed(2)}</span> more to your cart for <span className="font-bold">Free Shipping</span>!</p>
                <div className="w-full bg-primary-200 rounded-full h-2 overflow-hidden">
                  <div className="bg-primary-600 h-2 rounded-full transition-all duration-500" style={{ width: `${Math.min(100, (totalAmount / FREE_SHIPPING_THRESHOLD) * 100)}%` }}></div>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div key={`${item.id}-${item.color}-${item.size}`} className={`p-6 flex flex-col sm:flex-row gap-6 ${index !== items.length - 1 ? 'border-b border-gray-100' : ''}`}>
                  ...
                  <div className="flex-1 flex flex-col">
                    <div className="flex justify-between items-start gap-4">
                      <Link to={`/product/${item.id}`} className="font-semibold text-gray-900 hover:text-primary-600 text-lg leading-tight line-clamp-2">
                        {item.name}
                      </Link>
                    ...
                    <div className="mt-auto flex flex-wrap items-center justify-between gap-4 pt-4">
                      <div className="flex items-center border border-gray-200 rounded-lg bg-gray-50 p-1">
                        <button onClick={() => handleUpdateQuantity(item, item.quantity - 1)} className="w-8 h-8 flex items-center justify-center bg-white rounded shadow-sm text-gray-600 hover:text-primary-600 transition-colors disabled:opacity-50" disabled={item.quantity <= 1}>
                          <Minus className="w-4 h-4" />
                        </button>
                        <span className="w-12 text-center font-semibold text-gray-900">{item.quantity}</span>
                        <button onClick={() => handleUpdateQuantity(item, item.quantity + 1)} className="w-8 h-8 flex items-center justify-center bg-white rounded shadow-sm text-gray-600 hover:text-primary-600 transition-colors">
                          <Plus className="w-4 h-4" />
                        </button>
                      </div>
                      
                      <div className="flex gap-2">
                        <button onClick={() => dispatch(saveForLater(item))} className="text-sm font-medium text-gray-500 hover:text-primary-600 px-3 py-2 flex items-center gap-2 rounded-md hover:bg-gray-50 transition-colors">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                {savedForLater.map((item, index) => (
                  <div key={`${item.id}-${item.color}-${item.size}`} className={`p-4 flex gap-4 ${index !== savedForLater.length - 1 ? 'border-b border-gray-100' : ''}`}>
                    ...
                    <div className="flex-1">
                      <Link to={`/product/${item.id}`} className="font-semibold hover:text-primary-600 line-clamp-1">{item.name}</Link>
                      <div className="text-sm font-bold text-gray-900 my-1">${item.price.toFixed(2)}</div>
                      <div className="flex gap-3 mt-2">
                        <button onClick={() => dispatch(moveToCartFromSaved(item))} className="text-sm text-primary-600 font-medium hover:underline">Move to Cart</button>
        ...
        <div className="lg:w-1/3">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-24">
            ...
            <Link to="/checkout" className={`w-full bg-primary-600 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 hover:bg-primary-700 transition-colors shadow-lg shadow-primary-600/20 ${items.length === 0 ? 'opacity-50 pointer-events-none' : ''}`}>
